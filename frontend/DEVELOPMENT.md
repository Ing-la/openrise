...
├── lib/                     # 数据与工具
│   └── courses.ts          # 案例数据（COURSES、COURSES_CATALOG、ALL_COURSES）
├── public/
│   └── images/             # 图片资源
...
| 路径 | 页面 | 状态 | 说明 |
|------|------|------|------|
| `/` | 首页 | ✅ 完成 | Hero、精选案例、学员评价、共创团队、CTA |
| `/courses` | 案例库列表 | ✅ 完成 | 场景筛选、案例卡片网格 |
| `/courses/[slug]` | 案例详情 | ✅ 完成 | 大纲、讲师、包含内容 |
...
- **COURSES**：首页展示的 3 个核心案例（求职、论文、家庭教育）
- **COURSES_CATALOG**：案例库列表的 6 个补充案例
- **ALL_COURSES**：全部案例，用于列表页筛选
- **场景分类**：求职就业、论文调研、家庭教育、工作效率

### 修改案例时

1. 编辑 `lib/courses.ts`
2. 新增案例：在 `COURSES` 或 `COURSES_CATALOG` 中按 `Course` 接口添加
3. 新增场景：在 `courses/page.tsx` 的 `TOPICS` 中增加，并在 `courses.ts` 中为案例设置对应 `topic`
...
- `docs/OpenRise-产品核心理念.md`：产品理念与定位
...
| 日期 | 更新内容 |
|------|----------|
| 2024-03 | 初版：品牌 OpenRise、全站中文、全栈 Inter 字体、静态页面完成 |
